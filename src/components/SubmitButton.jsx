 ...
 */
export default function SubmitButton ({ children, className = '', disabled, id, isLoading, isValid, onClick, type, value }) {
  ...
  let classes = className.split(' ').concat([
    'form__submit',
    `${ctx.state.formName}__submit`,
    `form__submit--${isValid ? 'is' : 'not'}_valid`,
  ]);
